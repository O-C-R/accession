...
function getWordMatches(pattern, mode, extension, weights, max, imagesOnly) {
	...
	for (var i = 0; i < candidates.length; i++) {
		...
		var patternLev = lev.get(pattern, candidates[i].NLP.NLPString) / Math.max(pattern.length, candidates[i].NLP.NLPString.length) ;
		var posLev = lev.get(partsOfSpeech, candidates[i].NLP.PartsOfSpeech.join(" ")) / Math.max(partsOfSpeech.length, candidates[i].NLP.PartsOfSpeech.join(" ").length);
		var stressLev = lev.get(stresses, candidates[i].NLP.Stresses) / Math.max(stresses.length, candidates[i].NLP.Stresses.length);
		var syllablesLev = lev.get(syllables, candidates[i].NLP.Syllables) / Math.max(syllables.length, candidates[i].NLP.Syllables.length);
